...
export class UniswapConnector implements QuoteConnector {
  ...
  private readonly options: UniswapConnectorOptions;
  ...
  async getQuote(request: QuoteRequest): Promise<QuoteResult | null> {
    ...
    if (this.options.apiUrl) {
      ...
      try {
        ...
        const target = (data.router ?? zeroAddress) as Address;
        const callData = (data.calldata ?? "0x") as Hex;
        const gasEstimate = data.gasEstimate ? bigintFromDecimalString(data.gasEstimate) : 200_000n;
